...
void tq_weight_dequant(
    ...
    // Without this, <<<grid, block, smem>>> launches on the default stream
    // (stream 0) which is invisible to graph capture, causing the MoE
    // shared-scratch aliasing bug (varjoranta/turboquant-vllm#14).
    ...
    // The stream argument ensures kernels land on PyTorch's current stream.
