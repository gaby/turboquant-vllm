...
- **This plugin (`pip install turboquant-plus-vllm`)** — monkey-patches vLLM at import time via the `vllm.general_plugins` entry point. Validated on **vLLM 0.19–0.20**; enabled through the `TQ_WEIGHT_BITS` env var, includes the CUDA bs=1 fused-dequant kernel, fully standalone. **vLLM 0.25.x support is partial/early** (as of v0.13.10): startup is restored (0.25 exports `FusedMoE` as a non-class object, which crashed engine init before v0.13.10; online MoE compression is now skipped rather than crashing), and a small dense model decodes end-to-end (Qwen2.5-0.5B, GPU-verified eager + graphs), but a larger dense model that worked on 0.19 still hits a separate 0.25 break (`KeyError: 'weight'` in the model forward, e.g. Qwen3-8B). **Pin vLLM ≤ 0.20 for production** until 0.25 support is finished.
